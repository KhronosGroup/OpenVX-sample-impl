...
static void addCorner(vx_int32 y, int16x8_t *pvX, uint8x8_t *pvPred, vx_uint8 *pStrength, vx_size dst_capacity,
                      vx_size *num_corners, vx_array points)
{
    ...
    if (0 != vget_lane_u8(vPred, 0) && (*num_corners) < dst_capacity)
    {
        ...
        kp.error = 0.0f;
        (void)vxAddArrayItems(points, 1, &kp, sizeof(kp));
        *num_corners += 1;
    }
                         ...
                         vx_scalar s_num_corners, vx_border_t *bordermode)
{
    ...
    if (status == VX_SUCCESS)
    {
        ...
        {
            vx_int32 y, x;
            vx_int32 w8 = ((src_addr.dim_x - 2 * APERTURE) >> 3) << 3;
            ...
            for (y = APERTURE; y < (vx_int32)(src_addr.dim_y - APERTURE); y++)
            {
                for (vx_uint8 idx = 0; idx < 7; idx++)
                {
                    ...
                    vCur[idx] = vld1_u8((vx_uint8 *)src_base + (y - APERTURE + idx) * src_addr.stride_y);
                }
                for (x = 0; x < w8; x += 8)
                {
                    for (vx_uint8 idx = 0; idx < 7; idx++)
                    {
                        vNxt[idx] = vld1_u8((vx_uint8 *)src_base + (y - APERTURE + idx) * src_addr.stride_y + (x + 8) * src_addr.stride_x);
                    }
                    int16x8_t vX = vaddq_s16(vdupq_n_s16(x), vXStep);

                    memset(szStrength, 0, 8);
                    fast9CornersPerRow(vPrv, vCur, vNxt, &vX, &src_addr, tolerance, szStrength);
                    ...
                    if (do_nonmax && (0 != vget_lane_u64(vRetBit, 0)))
                    {
                        ...
                        if ((y - 1) >= APERTURE)
                        {
                            if (x != 0)
                            {
                                vNMPrv[0] = vld1_u8((vx_uint8 *)src_base + (y - APERTURE - 1) * src_addr.stride_y + (x - 8) * src_addr.stride_x);
                            }
                            else
                            {
                                vNMPrv[0] = vdup_n_u8(0);
                            }
                            vNMCur[0] = vld1_u8((vx_uint8 *)src_base + (y - APERTURE - 1) * src_addr.stride_y + x * src_addr.stride_x);
                            vNMNxt[0] = vld1_u8((vx_uint8 *)src_base + (y - APERTURE - 1) * src_addr.stride_y + (x + 8) * src_addr.stride_x);
                            ...
                            fast9CornersPerRow(vTmpPrv, vTmpCur, vTmpNxt, &vNMX, &src_addr, tolerance, szNMStrength);
                            ...
                            {
                                memset(szNMStrength, 0, 8);
                                fast9CornersPerRow(vNMPrv, vNMCur, vNMNxt, &vX, &src_addr, tolerance, szNMStrength);
                                vNMStrength = vld1_u8(szNMStrength);
                                vTmpPred = vcge_u8(vStrength, vNMStrength);
                                vPred = vand_u8(vPred, vTmpPred);
                                vRetBit = vreinterpret_u64_u8(vPred);
                        ...
                        if ((y + 1) < (src_addr.dim_y - APERTURE))
                        {
                            if (0 != vget_lane_u64(vRetBit, 0))
                            {
                                if (x != 0)
                                {
                                    vNMPrv[6] = vld1_u8((vx_uint8 *)src_base + (y + APERTURE + 1) * src_addr.stride_y + (x - 8) * src_addr.stride_x);
                                }
                                else
                                {
                                    vNMPrv[6] = vdup_n_u8(0);
                                }
                                vNMCur[6] = vld1_u8((vx_uint8 *)src_base + (y + APERTURE + 1) * src_addr.stride_y + x * src_addr.stride_x);
                                vNMNxt[6] = vld1_u8((vx_uint8 *)src_base + (y + APERTURE + 1) * src_addr.stride_y + (x + 8) * src_addr.stride_x);
                ...
                for (x = w8; x < (src_addr.dim_x - APERTURE); x++)
                {
