...
static vx_status Gaussian5x5_U8_S16(vx_image src, vx_image dst, vx_border_t *bordermode)
{
    ...
    vx_uint32 width = high_x;
    vx_uint32 height = high_y;

    vx_int16 * mid_dst = (vx_int16 *)malloc(width * height * sizeof(vx_int16));
    //gaussian5x5 hor
    vx_uint32 w8 = width / 8 * 8;

    for (y = low_y; y < high_y; ++y)
    {
        vx_uint8* srcp = (vx_uint8 *)src_base + y * srcStride;

        vx_int16* mid_dstp = (vx_int16 *)mid_dst + y * srcStride;
    ...
    for (y = low_y; y < high_y; ++y)
    {
        ...
        vx_int16* dstp = (vx_int16 *)dst_base + low_x + y * dstStride / 2;
        ...
        for (; x < width_x8; x += 8)
        {
            //HIGH DATA
            //top2
            int16x8_t data_high = vld1q_s16(top_src2);
            int16x8_t out_high = data_high;
            //top
            data_high = vld1q_s16(top_src1);
            out_high = vmlaq_s16(out_high, data_high, four);
            //mid
            data_high = vld1q_s16(mid_src);
            out_high = vmlaq_s16(out_high, data_high, six);
            //low
            data_high = vld1q_s16(bot_src1);
            out_high = vmlaq_s16(out_high, data_high, four);
            //low2
            data_high = vld1q_s16(bot_src2);
            out_high = vaddq_s16(out_high, data_high);

            vst1q_s16(dstp, vshrq_n_s16(out_high, 8));
