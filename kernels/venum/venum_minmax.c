...
vx_status vxMin(vx_image in0, vx_image in1, vx_image output)
{
    ...
    case VX_DF_IMAGE_S16:
        ...
        for (y = 0; y < height; y++)
        {
            vx_int16* src0p = (vx_int16 *)src_base[0] + y * width;
            vx_int16* src1p = (vx_int16 *)src_base[1] + y * width;
            vx_int16* dstp = (vx_int16 *)dst_base + y * width;
            for (x = 0; x < width; x+=8, src0p+=8, src1p+=8, dstp+=8)
            {
                const int16x8_t vsrc0 = vld1q_s16( src0p );
                const int16x8_t vsrc1 = vld1q_s16( src1p );

                vst1q_s16( dstp,vminq_s16( vsrc0, vsrc1 ) );
            }
        }
        break;
