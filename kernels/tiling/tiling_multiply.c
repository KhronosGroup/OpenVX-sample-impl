...
#define MULTIPLY_FLEXIBLE(low_y, low_x, high_y, high_x)                                        \
...
void Multiply_image_tiling_flexible(void * parameters[], void * tile_memory, vx_size tile_memory_size)
{
    ...
    if (ty == 0 && tx == 0)
    {
        MULTIPLY_FLEXIBLE(0, 0, vxTileHeight(out, 0), vxTileWidth(out, 0))     
    }
    else
    {
        MULTIPLY_FLEXIBLE(0, tx, ty, vxTileWidth(out, 0))
        MULTIPLY_FLEXIBLE(ty, 0, vxTileHeight(out, 0), vxTileWidth(out, 0))
    }
}
