...
void Addition_image_tiling_fast(void * VX_RESTRICT parameters[VX_RESTRICT], void * VX_RESTRICT tile_memory, vx_size tile_memory_size)
{
    ...
    {
        vx_uint8 *src0p = (vx_uint8 *)in_1->base[0] + in_1->tile_x + y * in_1->addr->stride_y;
        vx_uint8 *src1p = (vx_uint8 *)in_2->base[0] + in_2->tile_x + y * in_2->addr->stride_y;
        vx_uint8 *dstp = (vx_uint8 *)out->base[0] + out->tile_x + y * out->addr->stride_y;
        vx_int16 *src0p_16 = (vx_int16 *)in_1->base[0] + in_1->tile_x + y * in_1->addr->stride_y / 2;
...
#define ADD_SUB_FLEXIBLE(low_y, low_x, high_y, high_x, opmode, in_1_tile_x, in_2_tile_x, out_tile_x)        \
    ...
    {                                                                                                       \
        vx_uint8 *src0p = (vx_uint8 *)in_1->base[0] + in_1_tile_x + y * in_1->addr->stride_y;               \
        vx_uint8 *src1p = (vx_uint8 *)in_2->base[0] + in_2_tile_x + y * in_2->addr->stride_y;               \
        vx_uint8 *dstp = (vx_uint8 *)out->base[0] + out_tile_x + y * out->addr->stride_y;                   \
        vx_int16 *src0p_16 = (vx_int16 *)in_1->base[0] + in_1_tile_x + y * in_1->addr->stride_y / 2;        \
        vx_int16 *src1p_16 = (vx_int16 *)in_2->base[0] + in_2_tile_x + y * in_2->addr->stride_y / 2;        \
        vx_int16 *dstp_16 = (vx_int16 *)out->base[0] + out_tile_x + y * out->addr->stride_y / 2;            \
