...
static vx_bool read_pixel(void *base, vx_imagepatch_addressing_t *addr, vx_uint32 src_height, vx_uint32 src_width,
    vx_float32 x, vx_float32 y, vx_uint8 *pixel, vx_border_t borders)
{
...
#define REMAP(low_y, high_y, low_x)                                                                                              \
    ...
    {                                                                                                                            \
        ...
        {                                                                                                                        \
            ...
            if (policy == VX_INTERPOLATION_NEAREST_NEIGHBOR)                                                                     \
            {                                                                                                                    \
                read_pixel(src_base, in->addr, in->image.height, in->image.width, src_x + 0.5f, src_y + 0.5f, dst, borders);     \
                dst++;                                                                                                           \
            }                                                                                                                    \
            else if (policy == VX_INTERPOLATION_BILINEAR)                                                                        \
            {                                                                                                                    \
                ...
                defined &= read_pixel(src_base, in->addr, in->image.height, in->image.width, xf + 0, yf + 0, &tl, borders);      \
                defined &= read_pixel(src_base, in->addr, in->image.height, in->image.width, xf + 1, yf + 0, &tr, borders);      \
                defined &= read_pixel(src_base, in->addr, in->image.height, in->image.width, xf + 0, yf + 1, &bl, borders);      \
                defined &= read_pixel(src_base, in->addr, in->image.height, in->image.width, xf + 1, yf + 1, &br, borders);      \
                if (defined)                                                                                                     \
...
void Remap_image_tiling_flexible(void * VX_RESTRICT parameters[VX_RESTRICT], void * VX_RESTRICT tile_memory, vx_size tile_memory_size)
{
    vx_uint32 x = 0, y = 0;
