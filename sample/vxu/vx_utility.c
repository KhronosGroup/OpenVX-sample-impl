...
VX_API_ENTRY vx_status VX_API_CALL vxuWeightedAverageImage(vx_context context, vx_image img1, vx_scalar alpha, vx_image img2, vx_image output)
{
    vx_status status = VX_FAILURE;
    vx_graph graph = vxCreateGraph(context);
    if (vxGetStatus((vx_reference)graph) == VX_SUCCESS)
    {
        vx_node node = vxWeightedAverageImageNode(graph, img1, alpha, img2, output);
        if (vxGetStatus((vx_reference)node) == VX_SUCCESS)
        {
            status = setNodeTarget(node);
            if (status == VX_SUCCESS)
